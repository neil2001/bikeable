...
def test_score_is_clamped_to_ten_point_scale() -> None:
    ...
    score = score_road(perfect, get_profile(config), config=config)
...
def test_missing_traffic_does_not_drive_score_to_zero() -> None:
    ...
    assert score_road(features, get_profile(config), config=config) > 0.0
...
def test_score_road_returns_reasons() -> None:
    ...
    breakdown = score_road_detailed(features, get_profile(config), config=config)
